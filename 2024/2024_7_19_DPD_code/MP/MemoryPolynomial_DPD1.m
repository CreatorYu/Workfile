...
% path("E:\matlab\bin\CODE\mycode\test\Memorypolynomial",path);
%% （1）加载初始数据
%% Date 1
Center_F=2.5*1e6;
...
%% （5）基于memorypolynomial的DPD设计——包含偶阶项
K=7;Q=5;                                                                   %K表示记忆多项式阶数K实际为7阶，Q表示记忆深度
for i=1:10                                                                  %DPD迭代次数
    fprintf("\nIteration 次数 %d\n",i);
    if i==1                                                                 %表示初始状态，PD没有进行预失真处理。
        ...
        [V(:,i),Av_Power_diff(i)]=MP_DPD_Debug_odd(V(:,i-1),X1,Y(:,i-1),K,Q,G1(i-1));       %DPD算法——G1由PA输出Y与PA输入V求得
        %G1是PA增益值。
        Pin_V(i)=fun_Power_cal(V(:,i));
        fprintf("\n");
    end
     Y(:,i)=fun_Wiener_Model_LUT_cal(V(:,i), M, A, Plow, Pinm_dB, Num_section, Gstatic, PX);    %将中间信号V输入至PA模型，得到PA输出
...
% save("E:\matlab\bin\CODE\mycode\test\Memorypolynomial\Final_project\MPdata.mat");