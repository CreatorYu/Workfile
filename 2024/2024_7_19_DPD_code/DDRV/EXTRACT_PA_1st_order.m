clc
clear;
path('D:\MATLAB2020\bin\bin\MATLAB_CODE\CODE\mycode\data',path);
...
for k = 1:(K-1)/2+1
    for m = 1:Q+1
       U1(:,m+(k-1)*(Q+1)) = fun_delay(X1,m-1).*(abs(X1).^(2*(k-1)));
    end
end
U2 = zeros(N_y,N_x2); 
for k = 1:(K-1)/2
    for m = 1:Q
        U2(:,m+(k-1)*Q) = (X1.^2).*conj(fun_delay(X1,m)).*((abs(X1)).^(2*(k-1)));
    end
end
U = [U1 U2];
coefficiency = U\Y1;
...
fprintf("NMSE of Original and DDRV: %f (dB)\n",NMSE);
